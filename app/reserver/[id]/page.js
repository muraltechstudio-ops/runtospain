...
export default function ReservationPage() {
  ...
  if (!sortie) return (
    <div className="min-h-screen bg-brand-dark flex items-center justify-center text-white">
      <div className="text-center">
        <p className="text-2xl mb-4 font-display">{t('book.notFound')}</p>
        <Link href="/sorties" className="bg-brand-red px-8 py-3 rounded-xl font-bold transition hover:scale-105">{t('book.seeTrips')}</Link>
      </div>
    </div>
  )
  ...
  const validateForm = () => {
    if (!form.nom || !form.prenom || !form.email || !form.telephone || !form.depart) {
      setError(t('book.required')); return false
    }
    if (!/\S+@\S+\.\S+/.test(form.email)) {
      setError(t('book.invalidEmail')); return false
    }
    if (!form.cgv) {
      setError(t('book.acceptCGV')); return false
    }
  ...
  const handleStripe = async () => {
    ...
    try {
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({ sortieId: sortie.id, places: form.places, client: form, locale: language }),
      })
      const { sessionId } = await res.json()
      const stripe = await stripePromise
      await stripe.redirectToCheckout({ sessionId })
    } catch (e) {
      setError(t('book.paymentError'))
    }
    setLoading(false)
  }
    ...
    <>
      ...
      <main className="min-h-screen pt-32 pb-20 px-4 bg-brand-dark relative overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto relative z-10">
          ...
          <div className="mb-10">
            <h1 className="font-display text-7xl text-white leading-none mb-4 uppercase tracking-tighter">{t('book.title')}</h1>
          ...
          <div className="bg-brand-card rounded-3xl p-8 border border-white/5 mb-10 relative overflow-hidden group">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 relative z-10">
              <div><div className="text-gray-600 text-[9px] font-bold uppercase tracking-widest mb-2">{t('book.date')}</div><div className="text-white font-bold text-sm">{sortie.label}</div></div>
              <div><div className="text-gray-600 text-[9px] font-bold uppercase tracking-widest mb-2">{t('book.returnEstimate')}</div><div className="text-white font-bold text-sm">{sortie.retourEstime}</div></div>
              <div><div className="text-gray-600 text-[9px] font-bold uppercase tracking-widest mb-2">{t('book.availability')}</div><div className="text-green-400 font-bold text-sm">{sortie.placesRestantes} {t('badge.dispos')}</div></div>
              <div><div className="text-gray-600 text-[9px] font-bold uppercase tracking-widest mb-2">{t('book.price')}</div><div className="text-brand-orange font-display text-2xl">{sortie.prix}€</div></div>
            </div>
          </div>
            ...
            {step === 1 ? (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="font-bold text-white text-xl tracking-tight">{t('book.yourInfo')}</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Field label={t('book.firstName') + ' *'} name="prenom" placeholder="Ex: Marie" />
                  <Field label={t('book.lastName') + ' *'} name="nom" placeholder="Ex: Dupont" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Field label={t('book.email') + ' *'} name="email" type="email" placeholder="marie@exemple.fr" />
                  <Field label={t('book.phone') + ' *'} name="telephone" placeholder="06 12 34 56 78" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{t('book.departurePoint')} *</label>
                    <select value={form.depart} onChange={e => setForm({ ...form, depart: e.target.value })}
                      ...
                      <option value="" className="bg-brand-dark">{t('book.chooseDeparture')}</option>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{t('book.seats')}</label>
                    <select value={form.places} onChange={e => setForm({ ...form, places: parseInt(e.target.value) })}
                      ...
                      {Array.from({ length: Math.min(sortie.placesRestantes, 4) }, (_, i) => i + 1).map(n => (
                        <option key={n} value={n} className="bg-brand-dark text-sm">{n} {n > 1 ? t('sl.seats') : t('sl.seat')}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-8 border-t border-white/5">
                  <div className="flex flex-col">
                    <span className="text-gray-500 text-[10px] uppercase font-bold tracking-[0.2em] mb-1">{t('book.totalDue')}</span>
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-bold text-white text-xl tracking-tight uppercase">{t('book.payment')}</h2>
                  </div>
                  <button onClick={() => setStep(1)} className="text-gray-500 hover:text-white text-[10px] font-bold uppercase tracking-widest transition-colors">← {t('book.modify')}</button>
                </div>
                ...
                <div className="flex justify-between items-center bg-white/5 rounded-2xl p-6 border border-white/5">
                  <div>
                    ...
                    <div className="text-gray-500 text-[10px] font-bold uppercase tracking-widest">{form.places} {form.places > 1 ? t('sl.seats') : t('sl.seat')} · {form.depart}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-brand-orange font-display text-3xl leading-none mb-1">{total}€</div>
                    <div className="text-gray-600 text-[9px] font-bold uppercase tracking-tighter">{t('book.total')}</div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <button onClick={handleStripe} disabled={loading}
                    ...
                    {loading ? t('book.stripeConnecting') : <><CreditCard size={20} /> {t('book.payCard')}</>}
                  </button>

                  <div className="relative flex items-center gap-4 py-2">
                    <div className="flex-1 h-px bg-white/5" />
                    <span className="text-gray-600 text-[10px] font-bold uppercase tracking-widest">{t('book.or')}</span>
                    <div className="flex-1 h-px bg-white/5" />
                  </div>
                  ...
                  <div className="rounded-2xl overflow-hidden border border-white/5 p-4 bg-white/[0.02]">
                    <PayPalScriptProvider options={{ clientId: process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID, currency: 'EUR' }}>
                      <PayPalButtons
                        ...
                        onError={() => setError(t('book.paypalError'))}
                      />
                ...
                <div className="flex flex-col items-center gap-4 pt-4">
                  ...
                  <p className="text-gray-600 text-[9px] uppercase font-bold tracking-[0.2em]">{t('book.secureFooter')}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
